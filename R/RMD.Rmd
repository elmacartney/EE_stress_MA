...
```{r, message = FALSE, warning = FALSE, results = 'hide'}
#shifting up negative values by the lowest value in the raw data (all from Wang et al 2020): 
#ES 36 up by 29.40
#ES 37 up by 36.65
#ES 38 up by 80
#ES 39 up by 33.86
...
test <- dat %>%
  ...
  mutate(CS_mean = case_when(ES_ID == 36 ~ CC_mean+29.4)) %>%
  mutate(EC_mean = case_when(ES_ID == 36 ~ CC_mean+29.4)) %>%
  mutate(ES_mean = case_when(ES_ID == 36 ~ CC_mean+29.4)) %>%
  mutate(CC_mean = case_when(ES_ID == 37 ~ CC_mean+36.65))%>%
  mutate(CS_mean = case_when(ES_ID == 37 ~ CC_mean+36.65))%>%
  mutate(EC_mean = case_when(ES_ID == 37 ~ CC_mean+36.65))%>%
  mutate(ES_mean = case_when(ES_ID == 37 ~ CC_mean+36.65))%>%
  mutate(CC_mean = case_when(ES_ID == 38 ~ CC_mean+80)) %>%
  mutate(CS_mean = case_when(ES_ID == 38 ~ CC_mean+80)) %>%
  mutate(EC_mean = case_when(ES_ID == 38 ~ CC_mean+80)) %>%
  mutate(ES_mean = case_when(ES_ID == 38 ~ CC_mean+80)) %>%
  mutate(CC_mean = case_when(ES_ID == 39 ~ CC_mean+33.86))%>%
  mutate(CS_mean = case_when(ES_ID == 39 ~ CC_mean+33.86)) %>%
  mutate(EC_mean = case_when(ES_ID == 39 ~ CC_mean+33.86)) %>%
  mutate(ES_mean = case_when(ES_ID == 39 ~ CC_mean+33.86))
