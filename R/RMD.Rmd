...
``` {r, message = FALSE, warning = FALSE}
dat3<-dat %>% subset(Stress_duration < 4) 
dat3$Stress_duration<-as.factor(dat3$Stress_duration)
dat3 <- dat3 %>% mutate(Stress_duration = case_when(Stress_duration == 1 ~ "Acute", Stress_duration == 2 ~ "Chronic", Stress_duration == 3 ~ "Intermittent", Stress_duration == 4 ~ "Unkown"))

mod_S6 <-rma.mv(yi = lnRR_Sa, V = lnRRV_S, mod = ~Stress_duration-1, random = list(~1|Study_ID,                                                                                 ~1|ES_ID),
                test = "t",
                data = dat3)
summary(mod_S6) #unclear is significant. Need to sort out all the ages
r2_ml(mod_S6) #marginal R2 is very high
...
mod_E1 <- rma.mv(yi = lnRR_Ea, V = lnRRV_E, mod = ~Type_learning-1, random = list(~1|Study_ID,                                                                          ~1|ES_ID),
                 test = "t",
                 data = dat)
...
#dat1 <- dat1 %>% mutate(Learning_vs_memory = case_when(Learning_vs_memory == 1 ~ "Learning", Learning_vs_memory == 2 ~ "Memory"))

mod_E2 <-  rma.mv(yi = lnRR_Ea, V = lnRRV_E, mod = ~Learning_vs_memory-1, random = list(~1|Study_ID,                                                                                       ~1|ES_ID),
                  test = "t",
                  data = dat1)

summary(mod_E2) #memory but nor learning affected
...
mod_E3 <- rma.mv(yi = lnRR_Ea, V = lnRRV_E, mod = ~ Appetitive_vs_aversive-1, random = list(~1|Study_ID,                                                                              ~1|ES_ID),
                 test = "t",
                 data = dat2)
